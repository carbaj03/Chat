...
class ModelApi {
  ...
  suspend fun models(): List<Model> = catch(onError = { raise(DomainError.UnknownDomainError(it)) }) {
    val response = client.get("models")
    response.body()
  }

  context(Raise<DomainError>)
  suspend fun model(id: String): Model = catch(onError = { raise(DomainError.UnknownDomainError(it)) }) {
